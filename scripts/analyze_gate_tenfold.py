...
DEFAULT_OUTPUT_DIR = os.path.join(PROJECT_ROOT, "analysis_outputs", "gate_tenfold")
...
DEFAULT_OKX_SCANNED_FILE = os.path.join(
    PROJECT_ROOT,
    "analysis_outputs",
    "okx_tenfold",
    "scanned_symbols.txt",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Analyze Gate futures candle history for symbols that meet the project's "
            "tenfold condition."
        ),
    )
    parser.add_argument(
        "symbols",
        nargs="*",
        help="Symbols like APE, ZBT, RAVE or full contract names like APE_USDT.",
    )
    parser.add_argument(
        "--host",
        default="fx-api.gateio.ws",
        help="Gate public API host. Default: fx-api.gateio.ws",
    )
    parser.add_argument(
        "--bar",
        default="1H",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Default: analysis_outputs/gate_tenfold/result.json"
        ),
    )
    parser.add_argument(
        "--csv-out",
        help=(
            ...
            "Default: analysis_outputs/gate_tenfold/result.csv"
        ),
    )
    parser.add_argument(
        "--chart-dir",
        help=(
            ...
            "Default: analysis_outputs/gate_tenfold/charts"
        ),
    )
    parser.add_argument(
        "--chart-window",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Default: analysis_outputs/gate_tenfold/scanned_symbols.txt"
        ),
    )
    parser.add_argument(
        "--hit-list-file",
        ...
        help=(
            ...
            "Default: analysis_outputs/gate_tenfold/tenfold_hit_symbols.txt"
        ),
    )
    parser.add_argument(
        "--progress-file",
        ...
        help=(
            ...
            "Default: analysis_outputs/gate_tenfold/scan_progress.jsonl"
        ),
    )
    parser.add_argument(
        "--rescan",
    ...
    parser.add_argument(
        "--okx-scanned-file",
        default=DEFAULT_OKX_SCANNED_FILE,
        help=(
            "Path to OKX scanned_symbols.txt. "
            "Gate symbols whose base coin already appeared in this OKX file will be skipped. "
    ...
    parser.add_argument(
        "--no-skip-okx-scanned",
        action="store_true",
        help="Do not filter out Gate symbols that were already scanned in OKX.",
    )
    parser.add_argument(
        "--all-futures",
        action="store_true",
        help="Analyze all live Gate USDT futures contracts.",
    )
    return parser.parse_args()


def normalize_symbol(symbol: str) -> str:
    value = symbol.strip().upper().replace("/", "_").replace("-", "_")
    if value.endswith("_USDT"):
        return value
    if value.endswith("USDT") and "_" not in value and len(value) > 4:
...
def run_curl(url: str) -> dict[str, Any]:
    ...
    for attempt in range(3):
        ...
        if not stderr and not result.stdout.strip():
            stderr = "empty response"
...
def fetch_candles(
    ...
    interval = interval_map.get(bar, bar.lower())
    base_url = f"https://{host}/api/v4/futures/usdt/candlesticks"
    ...
    for _ in range(max_pages):
        from_ts = max(0, to_ts - (page_limit * window_seconds))
        url = (
            f"{base_url}?contract={inst_id}&interval={interval}"
        ...
        for row in payload:
            ...
            ts_raw = row.get("t")
            try:
                ts_seconds = int(float(str(ts_raw)))
            ...
            seen.add(ts_ms)
            rows.append(row)
            if oldest_ts is None or ts_seconds < oldest_ts:
                oldest_ts = ts_seconds

        if len(rows) == before_count or oldest_ts is None:
            break
        to_ts = oldest_ts - 1
        if to_ts <= 0:
            break
    ...
    for row in rows:
        try:
            ...
            candles.append(
                Candle(
                    ts=ts_ms,
                    open=float(str(row.get("o", 0) or 0)),
                    high=float(str(row.get("h", 0) or 0)),
                    low=float(str(row.get("l", 0) or 0)),
                    close=float(str(row.get("c", 0) or 0)),
                    volume=float(str(row.get("v", 0) or 0)),
                ),
            )
        except (TypeError, ValueError):
            continue
...
def fetch_instruments(host: str, market: str) -> list[InstrumentRecord]:
    del market
    url = f"https://{host}/api/v4/futures/usdt/contracts"
    payload = run_curl_with_fallback(url, host)
    if not isinstance(payload, list):
        raise RuntimeError(f"Gate returned unexpected contracts payload: {type(payload).__name__}")

    records: list[InstrumentRecord] = []
    for row in payload:
        if not isinstance(row, dict):
            continue
        inst_id = str(row.get("name", ""))
        if not inst_id:
            continue
        records.append(
            InstrumentRecord(
                inst_id=inst_id,
                base_ccy=inst_id.replace("_USDT", ""),
                quote_ccy="USDT",
                settle_ccy="USDT",
...
def fetch_tickers(host: str, market: str) -> list[TickerRecord]:
    del market
    url = f"https://{host}/api/v4/futures/usdt/tickers"
    payload = run_curl_with_fallback(url, host)
    if not isinstance(payload, list):
        raise RuntimeError(f"Gate returned unexpected tickers payload: {type(payload).__name__}")

    records: list[TickerRecord] = []
    for row in payload:
        if not isinstance(row, dict):
            continue
        inst_id = str(row.get("contract", ""))
        if not inst_id:
            continue
        last = float(str(row.get("last", "0") or "0"))
        change24h = float(str(row.get("change_percentage", "0") or "0"))
        volume_24h_quote = float(str(row.get("volume_24h_quote", "0") or "0"))
        records.append(
            TickerRecord(
                inst_id=inst_id,
                last_price=last,
                today_change_percent=change24h,
...
def select_all_symbols(host: str, market: str) -> list[str]:
    ...
    def is_live_usdt(record: InstrumentRecord) -> bool:
        return record.state.lower() == "trading" and record.settle_ccy.upper() == "USDT"

    ranked = sorted(record.inst_id for record in instruments if is_live_usdt(record))
    return ranked
...
def print_report(inst_id: str, summary: dict[str, Any], threshold: float) -> None:
    ...
    if summary["hit_count"] == 0:
        ...
        if highest_candidate["next_candle_ohlc"] is not None:
            next_open, next_high, next_low, next_close = highest_candidate["next_candle_ohlc"]
            print(
                "- 该根后的第二根K线OHLC: "
                f"O {next_open:.6f} | H {next_high:.6f} | "
                f"L {next_low:.6f} | C {next_close:.6f}",
            )
        print()
        return
    ...
    for index, event in enumerate(summary["events"], start=1):
        ...
        if event["next_candle_ohlc"] is not None:
            next_open, next_high, next_low, next_close = event["next_candle_ohlc"]
            print(
                "     - 命中后第二根K线OHLC: "
                f"O {next_open:.6f} | H {next_high:.6f} | "
                f"L {next_low:.6f} | C {next_close:.6f}",
        ...
        if event["next_candle_high_vs_threshold_percent"] is not None:
            high_distance = float(event["next_candle_high_vs_threshold_percent"])
            print(
                "     - 第二根最高点距离10倍价: "
                f"{high_distance:+.4f}个点",
            )
        if event["next_candle_low_vs_threshold_percent"] is not None:
            low_distance = float(event["next_candle_low_vs_threshold_percent"])
            print(
                "     - 第二根最低点距离10倍价: "
                f"{low_distance:+.4f}个点",
            )
    print()
                ...
                writer.writerow(
                    ...
                    + [
                        ...
                        (
                            ""
                            if not next_ohlc
                            else f"{float(next_ohlc[0]):.6f}"
                        ),
                        (
                            ""
                            if not next_ohlc
                            else f"{float(next_ohlc[1]):.6f}"
                        ),
                        (
                            ""
                            if not next_ohlc
                            else f"{float(next_ohlc[2]):.6f}"
                        ),
                        (
                            ""
                            if not next_ohlc
                            else f"{float(next_ohlc[3]):.6f}"
                        ),
                        (
                            ""
...
def main() -> int:
    ...
    if args.all_futures:
        ...
        print(f"使用 Gate 全量合约模式，共选中 {len(symbols)} 个币进行分析。")

    if not symbols:
        raise SystemExit("Please pass Gate symbols, or use --all-futures.")

    pre_scanned = set() if args.rescan else load_scanned_symbols(scanned_file)
    normalized_scan_targets = [normalize_symbol(raw_symbol) for raw_symbol in symbols]
    okx_scanned_bases: set[str] = set()
    okx_filtered_symbols: list[str] = []
    if not args.no_skip_okx_scanned:
        okx_scanned_symbols = load_scanned_symbols(args.okx_scanned_file)
        okx_scanned_bases = {
            extract_base_symbol(symbol)
            for symbol in okx_scanned_symbols
        }
        if okx_scanned_bases:
            filtered_targets: list[str] = []
            for inst_id in normalized_scan_targets:
                if extract_base_symbol(inst_id) in okx_scanned_bases:
                    okx_filtered_symbols.append(inst_id)
                else:
                    filtered_targets.append(inst_id)
            normalized_scan_targets = filtered_targets
    ...
    if okx_filtered_symbols:
        print(
            f"已按 OKX 扫描记录过滤 {len(okx_filtered_symbols)} 个 Gate 币"
            f"（OKX记录: {args.okx_scanned_file}）。",
        )
    if args.rescan:
    ...
    try:
        for index, inst_id in enumerate(pending_symbols, start=1):
            ...
            print(
                f"========== 开始扫描 Gate [{index}/{len(pending_symbols)}] {inst_id} ==========",
            )
            try:
